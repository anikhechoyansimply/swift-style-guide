...
This style guide is meant to keep the code in our projects clear and consistent — even though we have many developers working on them.
...
**Preferred**:
```swift
func statePickerView(_ statePickerView: StatePickerView, didSelectState state: String)
func statePickerViewShouldReload(_ statePickerView: StatePickerView) -> Bool
```

**Not Preferred**:
```swift
func didSelectState(statePicker: StatePickerViewController, state: String)
func statePickerShouldReload() -> Bool
```
...
* There should be no blank lines after an opening brace or before a closing brace for `if`/`else`/`switch`/`while` etc. In case of type declaration (`struct`/`class`/`enum`/`extension` etc.) there should be one blank line after an opening brace and one before a closing brace.

* Closing parentheses may appear on a line by themselves.
